...
\end{equation}
\comment{Bernardo: I am not sure of what Equation~\ref{eq:wcet} represents. What is $T_i$? You introduced $T$ as the set of tasks, while the tasks inside the set are presented as $\tau_i$, hence I am not sure of what is $T_i$. On the other hand, the WCET is the maximum possible computation time that your job could suffer and, in general, is not known. You only know the maximum computation time that was observed during different executions of your program  and from that you could try to infer something as you mentioned in the next paragraph.}
...
\comment{Bernardo: Here you introduce $T_i$ as the task period, however it is evident that the period does not have anything to do with the WCET.}
...
\comment{Bernardo: I will include the paragraph talking about SCHED\_DEADLINE in the next section. SCHED\_DEADLINE is a scheduling policy, as SCHED\_FIFO or SCHED\_RR, for the Linux kernel and it is based on EDF. Hence, I believe that it has more sense to describe it in the Resource Reservation Scheduling.}
...
\comment{Bernardo: I removed the RR acronym. As a paper reviewer pointed out to us, the real-time systems community uses RR for round-robin, hence it could be misleading to use it also for resource reservations.}
...
to guarantee a 100\% precision on the WCET value. 
\comment{Bernardo: I am not sure about this statement, the real--time OS kernels avoid unbounded latencies (reducing them to a minimum) and, as a result, they provides fast ``worst case'' times, but I am not sure that they guarantee that precision. Consider that the result of a job depends on the input, so you will need to provide all the possible inputs to the program in order to expect the effective WCET.}
